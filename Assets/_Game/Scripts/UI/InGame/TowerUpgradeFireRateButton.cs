...
public class TowerUpgradeFireRateButton : MonoBehaviour
{
                     private Tower      tower;
    ...
    private void Awake()
    {
        ...
        button.onClick.AddListener(RateUpgradeButtonClicked);
    }
    ...
    {
        factor = 20f * tower.fireRateUpgradeLevel;
        Debug.Log("factor: " + factor.ToString());
        cost = (int)((tower.towerCost / 100f) * factor);
        costText.text = cost.ToString();
        if (cost <= MoneyManager.Instance.currentGold && tower.fireRateUpgradeLevel < 5 && !isDisabled)
        {
        ...
        switch (tower.fireRateUpgradeLevel)
        {
            case 2:
                level1.enabled = true;
                break;
    ...
    private void RateUpgradeButtonClicked()
    {
